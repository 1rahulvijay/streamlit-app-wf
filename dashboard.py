...
# ============ CUSTOM CSS - ENTERPRISE REACT NATIVE STYLE ============
st.markdown(
    """
<style>
    /* Import Fonts */
    ...
    .bg-error-light { background-color: #FEE2E2; color: #991B1B; }
...
# Mapping old constants to new theme for backward compatibility during refactor
NAVY = DARK
NAVY_LIGHT = PRIMARY
BLUE = PRIMARY
CYAN = "#0EA5E9"
...
chart_colors = [PRIMARY, ACCENT, "#3B82F6", "#8B5CF6", "#EC4899", "#14B8A6"]

# ============ COMPONENT FUNCTIONS ============


def render_kpi_card(label, value, change, trend, format_type="number", icon="📊"):
    """Render KPI card with new Enterprise styling"""
    ...
    if trend == "up":
        ...
        trend_bg = "bg-success-light"
        trend_icon = "↗"
    else:
        trend_color = "text-error"
        trend_bg = "bg-error-light"
        trend_icon = "↘"
    ...
    <div class="kpi-card animate-enter">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            ...
            <div class="kpi-icon bg-blue-light" style="color: {PRIMARY}; width: 40px; height: 40px; font-size: 1.2rem;">
                {icon}
            </div>
        </div>
        <div style="margin-top: 1rem; display: flex; align-items: center; gap: 8px;">
            <div class="kpi-trend {trend_bg} {trend_color}">
                {trend_icon} {abs(change)}%
            </div>
            <span style="font-size: 0.75rem; color: #94A3B8;">vs last period</span>
        </div>
    </div>
    """,
        unsafe_allow_html=True,
...
def render_summary_card(summary):
    """Render executive summary card with new visuals"""
    st.markdown(
        f"""
    <div class="summary-card animate-enter">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            ...
            <span class="summary-card-title" style="margin: 0; color: {ACCENT};">{summary["title"]}</span>
        </div>
        <p class="summary-card-content" style="margin-bottom: 20px;">{summary["content"]}</p>
        <div style="margin-top: auto; padding-top: 1rem; border-top: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center;">
            <div>
                <div style="font-size: 0.75rem; opacity: 0.7;">KEY METRIC</div>
                <div style="font-family: 'Outfit', sans-serif; font-size: 1.25rem; font-weight: 700;">{summary["metric"]}</div>
            </div>
            <div style="background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">
...
def create_workflow_bar_chart(data):
    ...
    fig.update_layout(
        ...
        font=dict(family="Inter, sans-serif", size=11, color=SECONDARY),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
        ...
        yaxis=dict(gridcolor="rgba(0,0,0,0.05)", gridwidth=1, zeroline=False),
        bargap=0.2,
        bargroupgap=0.05,
        hovermode="x unified",
    )
...
def create_department_pie_chart(data):
    ...
    fig.add_annotation(
        ...
        x=0.5,
        y=0.5,
        font=dict(size=14, weight="bold", family="Outfit, sans-serif", color=DARK),
        showarrow=False,
    )
...
def create_gearing_area_chart(data):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            marker=dict(size=8, color=SURFACE, line=dict(width=2, color=PRIMARY)),
        )
    )
    fig.add_trace(
        go.Scatter(
    ...
    fig.update_layout(
        ...
        margin=dict(l=20, r=20, t=30, b=20),
        xaxis=dict(gridcolor="rgba(0,0,0,0.05)", showgrid=False),
        yaxis=dict(gridcolor="rgba(0,0,0,0.05)", range=[0, 3], zeroline=False),
        hovermode="x unified",
    )
    return fig
...
def create_productivity_bar_chart(data):
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(l=20, r=20, t=30, b=20),
        xaxis=dict(gridcolor="rgba(0,0,0,0.05)", range=[0, 100], showgrid=False),
        yaxis=dict(gridcolor="rgba(0,0,0,0.05)", showgrid=False),
        bargap=0.4,
    )
    return fig
...
def create_tech_adoption_chart(data):
    ...
    colors = [
        SUCCESS if a >= 80 else PRIMARY if a >= 60 else ERROR for a in data["Adoption"]
    ]
    fig = go.Figure(
        go.Bar(
            x=data["Tool"],
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor="rgba(0,0,0,0.05)", tickangle=-45, showgrid=False),
        yaxis=dict(gridcolor="rgba(0,0,0,0.05)", range=[0, 110], showgrid=False),
    )
    return fig
...
def create_performance_area_chart(data):
    ...
    fig.update_layout(
        ...
        margin=dict(l=20, r=20, t=30, b=20),
        xaxis=dict(gridcolor="rgba(0,0,0,0.05)", showgrid=False),
        yaxis=dict(gridcolor="rgba(0,0,0,0.05)", tickformat="$,.0f", zeroline=False),
        hovermode="x unified",
    )
    return fig


def create_line_chart(data, x_col, y_col, name, color=PRIMARY):
    """Create simple line chart"""
    # Parse color if it is a hex string
    if isinstance(color, str) and color.startswith("#"):
        rgb = tuple(int(color.lstrip("#")[i : i + 2], 16) for i in (0, 2, 4))
        fill_color = f"rgba{rgb + (0.1,)}"
    else:
        fill_color = "rgba(37, 99, 235, 0.1)"  # Default fallback
    ...
    fig = go.Figure(
        go.Scatter(
            ...
            marker=dict(size=8, color=SURFACE, line=dict(width=2, color=color)),
            fill="tozeroy",
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            @media print {{
                ...
                .summary-box {{ color: black !important; background: #F8FAFC !important; border: 1px solid #E2E8F0; }}
                .summary-title {{ color: #0F172A !important; }}
            }}
        </style>
    </head>
    <body>
...
# ============ PAGE: PRODUCTIVITY ============
    ...
    with col1:
        st.markdown(
            f"""
        <div class="insight-card animate-enter">
            <h4>Top Performing Team</h4>
            <div class="insight-value" style="color: {PRIMARY};">{top_performer["Department"]}</div>
    ...
    with col3:
        ...
        <div class="insight-card animate-enter">
            ...
            <div class="insight-value" style="color: {SECONDARY};">{total_employees}</div>
...
# ============ PAGE: TECH ADOPTION ============
    ...
    with col2:
        ...
        st.markdown(
            f"""
        <div class="insight-card animate-enter">
            <h4 style="color: {PRIMARY};">✅ Top Performers</h4>
...
# ============ PAGE: EXECUTIVE SUMMARY ============
    ...
    st.markdown(
        f"""
    <div style="background: linear-gradient(135deg, {DARK} 0%, {NAVY_LIGHT} 100%); color: white; border-radius: 1rem; padding: 2.5rem; margin-bottom: 2rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
        <p style="color: {ACCENT}; font-size: 14px; margin-bottom: 12px; font-weight: 600; letter-spacing: 0.05em; text-transform: uppercase;">📅 {current_date}</p>
        <h2 style="color: white; margin-bottom: 16px; font-size: 2rem; font-weight: 700;">Monthly Executive Report</h2>
        <p style="color: rgba(255,255,255,0.8); max-width: 800px; line-height: 1.7; font-size: 1.1rem;">
    ...
    col1, col2, col3, col4 = st.columns(4)
    stats = [
        ("📈", kpi_data["workflow_count"]["value"], "Total Workflows", PRIMARY),
        ("📄", kpi_data["volume_gearing_ratio"]["value"], "Gearing Ratio", ACCENT),
        ("👥", f"{kpi_data['productivity']['value']}%", "Productivity", SUCCESS),
        ("💻", f"{kpi_data['tech_adoption']['value']}%", "Tech Adoption", "#8B5CF6"),
    ]
